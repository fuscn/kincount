<template>
  <div class="purchase-return-form">
    <van-nav-bar :title="isEdit ? '编辑采购退货单' : '新建采购退货单'" fixed placeholder left-text="取消" right-text="保存"
          ...
          <van-radio-group v-model="sourceType" direction="horizontal" class="source-type-group">
            <van-radio name="order">
              <div class="radio-label">
                采购订单<br>
                <span class="radio-desc">入库前退货</span>
              </div>
            </van-radio>
            <van-radio name="stock">
              <div class="radio-label">
                采购入库单<br>
                <span class="radio-desc">入库后退货</span>
              </div>
            </van-radio>
          </van-radio-group>

          <!-- 源单选择 -->
          <van-field v-model="sourceForm.source_name" :label="sourceType === 'order' ? '采购订单' : '采购入库单'"
            placeholder="请选择源单" is-link readonly @click="showSourcePicker = true"
            :rules="[{ required: true, message: '请选择源单' }]" />

          <van-field v-model="sourceForm.supplier_name" label="供应商" readonly />

          <van-field v-model="sourceForm.order_no" :label="sourceType === 'order' ? '订单编号' : '入库单号'" readonly />
        </div>
    ...
    <van-popup v-model:show="showSourcePicker" position="bottom" :style="{ height: '70%' }"
      ...
      <div class="picker-header">
        <van-nav-bar :title="sourceType === 'order' ? '选择采购订单' : '选择采购入库单'" left-text="取消"
          @click-left="closeSourcePicker" />
        <van-search v-model="sourceSearch" :placeholder="sourceType === 'order' ? '搜索订单编号/供应商名称' : '搜索入库单号/供应商名称'"
...
// 表单数据
const form = reactive({
  source_type: 'order', // order:采购订单, stock:采购入库单
  source_id: '',
  warehouse_id: '',
  warehouse_name: '',
...
const getSourceCellLabel = (source) => {
  const supplierName = source.supplier_name || source.supplier?.name || '无'
  const amount = source.total_amount || 0
  const statusText = getOrderStatusText(source.status)
  return `供应商: ${supplierName} | 金额: ¥${amount} | 状态: ${statusText}`
}
...
const initForm = async () => {
  if (isEdit) {
    ...
    try {
      // 注意：这里需要调用采购退货的API，但我们的store中已经有了相关函数
      await purchaseStore.loadReturnDetail(id)
      const returnDetail = purchaseStore.currentReturn

      if (returnDetail) {
        // 设置退货基本信息
        form.source_id = returnDetail.purchase_order_id || returnDetail.purchase_stock_id
        ...
        // 设置源单类型
        if (returnDetail.purchase_order_id) {
          sourceType.value = 'order'
          await loadSourceDetail(returnDetail.purchase_order_id, 'order')
        } else if (returnDetail.purchase_stock_id) {
          sourceType.value = 'stock'
          await loadSourceDetail(returnDetail.purchase_stock_id, 'stock')
        }
...
const buildSubmitData = () => {
  ...
  // 根据源单类型设置不同的source字段
  if (sourceType.value === 'order') {
    submitData.purchase_order_id = form.source_id
    submitData.purchase_stock_id = null
  } else {
    submitData.purchase_order_id = null
    submitData.purchase_stock_id = form.source_id
  }
  ...
  console.log('源单信息:', {
    ...
    源单ID: form.source_id,
    采购订单ID: submitData.purchase_order_id,
    采购入库单ID: submitData.purchase_stock_id
  })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const result = await purchaseStore.addReturn(submitData)

    console.log('Store返回结果:', result)

    if (result && result.code === 200) {
      showSuccessToast('采购退货单创建成功')

      // 如果后端返回了退货单ID，跳转到详情页
      if (result.data && result.data.id) {
        const returnId = result.data.id
        setTimeout(() => {
          router.push(`/purchase/return/detail/${returnId}`)
        }, 1500)
        ...
        setTimeout(async () => {
          // 刷新退货单列表
          await purchaseStore.loadReturnList({ page: 1, limit: 10 })
