<template>
  <div class="purchase-inbound-page">
    ...
    <van-form ref="formRef" class="form-container">
      ...
      <!-- 空状态 -->
      <van-empty
        v-if="form.items.length === 0 && !loadingOrder"
        description="请先选择采购订单"
        class="empty-state"
      />
    </van-form>
    ...
    >
      <div class="order-picker">
        ...
        />
        <van-list
          v-model:loading="loadingOrderList"
          :finished="finishedOrderList"
          finished-text="没有更多订单"
          @load="loadOrderList"
        >
          <van-cell
  ...
  try {
    ...
    form.items = (orderData.items || []).map(item => {
      ...
      return {
        ...
        sku_name: item.sku?.name || item.sku_name,
        spec_text: item.sku?.spec_text || item.spec_text,
