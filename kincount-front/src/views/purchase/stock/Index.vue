<template>
  <div class="purchase-stock-index">
    <!-- 导航栏 - 使用fixed布局 -->
    <van-nav-bar title="采购入库单管理" fixed placeholder left-text="返回" left-arrow @click-left="$router.back()" />
    ...
    <van-pull-refresh v-model="refreshing" @refresh="handleRefresh">
      <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多入库单了" @load="handleLoadMore" :immediate-check="false">
        <!-- 入库单项 -->
        <van-cell-group class="stock-list">
          <van-cell v-for="item in stockList" :key="item.id" :title="`入库单号：${item.stock_no || '未生成'}`" :label="getStockLabel(item)" @click="handleViewDetail(item)" is-link>
            <template #extra>
        ...
        <!-- 空状态 -->
        <van-empty v-if="!loading && !refreshing && stockList.length === 0" image="search" description="暂无入库单数据" />
      </van-list>
    </van-pull-refresh>
...
const getStockLabel = (item) => {
  const labels = []
  if (item.supplier?.name) labels.push(`供应商：${item.supplier.name}`)
...
const loadStockList = async (isRefresh = false) => {
  ...
  try {
    ...
    const res = await purchaseStore.loadStockList(params)
    
    const currentList = purchaseStore.stockList || []
    const currentDataLength = res?.data?.list?.length || currentList.length
    
    if (currentDataLength < pagination.value.page_size) {
      finished.value = true
    } else {
      if (!isRefresh) {
